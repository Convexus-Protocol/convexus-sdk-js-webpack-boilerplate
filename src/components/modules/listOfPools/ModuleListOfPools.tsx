...
const factoryContract = new Contract(factoryAddress, IConvexusFactory, iconService, debugService, networkId);
...
export const ModuleListOfPools = () => {
  ...
  <div className={appStyles.module}>
    <h1><pre>List of active pools</pre></h1>
    <p>Number of pools: {poolsState.length}</p>
    <p>Pools addresses:</p>
    {poolsState.map(poolAddress => (
      <p>- <a href={"https://tracker.berlin.icon.community/contract/" + poolAddress}>{poolAddress}</a></p>
    ))}
  </div> 
  )
}